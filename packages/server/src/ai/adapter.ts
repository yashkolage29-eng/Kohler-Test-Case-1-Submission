...
export interface AiAdapter {
  ...
  decor(text: string, fixtures: DecorFixture[]): Promise<AiResult<DecorProposal>>;
...
const DECOR_PROMPT =
  ...
  `item color is optional. metal must be exactly one of chrome, brass, black, nickel (gold → brass, silver → chrome). Aim for 6-12 items. At most ${MAX_DECOR_ITEMS} items and at most ${MAX_DECOR_LIGHTS} of ${LIGHT_DECOR_TYPES.join("/")}. ` +
...
function extractJson(content: string): unknown {
  try {
    // Reasoning models may prefix their answer with a <think>…</think> block.
    return JSON.parse(content.replace(/<think>[\s\S]*?<\/think>/gi, "").trim()) as unknown;
  } catch {
    return null;
  }
}
...
export function createAiAdapter(env: AiEnv): AiAdapter {
  ...
  return {
    ...
    async decor(text, fixtures) {
      ...
      const key = JSON.stringify([text.trim().toLowerCase(), fixtures.map((f) => `${f.fixtureClass}:${f.modelId}`).sort()]);
      ...
      const result = await nimRequest(nim, JSON.stringify({ brief: text, fixtures }), DECOR_PROMPT, 1200, "decor");
    ...
    async photoProposal(imageDataUrl, room) {
      ...
      const context = JSON.stringify({ room: { walls, openings: room.openings } });
      const result = await nimRequest(
        { ...nim, nimModel: env.nimVisionModel },
        [
